...
function renderSummary(r) {
  ...
  $("summary").innerHTML = `
    ...
    <div class="summary-grid">
      ...
      <div class="stat"><div class="num">${gpa.gpa.toFixed(2)}</div><div class="lbl">計算GPA（HTMLのGP列を使用・GPなし科目は除外）</div><div class="lbl">対象単位数 ${fmt(gpa.credits)} / GPT ${fmt(gpa.points)}</div></div>
    </div>`;
}
...
function renderMissing(r) {
  ...
  const rows = items.map(m => {
    ...
    const namedRows = namedMissing.map(x => `
      <tr class="named-row">
        <td><span class="must-badge">必須科目</span>${escapeHtml(x.label)}（${fmt(x.short)}単位）</td>
        <td class="num"></td>
        <td class="num"></td>
        <td class="num"></td>
      </tr>`).join("");
