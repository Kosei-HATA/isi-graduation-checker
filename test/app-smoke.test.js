...
test("app.js: DOMスタブ上で起動し、実HTMLのチェックが一連のレンダリングを実行できる", async t => {
  ...
  assert.match(els.get("summary").innerHTML, /3\.66/);
  assert.match(els.get("summary").innerHTML, /対象単位数 109\.5 \/ GPT 401/);
  ...
  assert.match(els.get("missing").innerHTML, /共創プロジェクト（1単位）/);
